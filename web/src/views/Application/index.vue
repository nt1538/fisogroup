<template>
  ...
  onMounted(async () => {
    try {
      const lifeRes = await axios.get('http://localhost:5000/api/orders/life?status=in_progress');
      const annuityRes = await axios.get('http://localhost:5000/api/orders/annuity?status=in_progress');
