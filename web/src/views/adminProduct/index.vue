      ...
      <table class="grid">
        <thead>
          <tr>
            ...
            <th>Renewals</th>
            <!-- NEW (life only) -->
            <th v-if="mode==='life'">Agent Excess</th>
            <th v-if="mode==='life'">Agent Renewals</th>
        ...
        <tbody>
          <tr v-for="row in rows" :key="rowKey(row)">
            ...
            <!-- NEW inputs (life only) -->
            <td v-if="mode==='life'">
              <input v-model.number="row.agent_excess_rate" type="number" step="0.01" />
            </td>
            <td v-if="mode==='life'">
              <input v-model.number="row.agent_renewal_rate" type="number" step="0.01" />
            </td>
          ...
          <tr v-if="!rows.length">
            <td :colspan="colCount" style="text-align:center; padding:16px;">No products</td>
          </tr>
        </tbody>
      </table>
    </div>
...
// column count for "No products" row
const colCount = computed(() => (mode.value === 'life' ? 10 : 8))
...
async function load() {
  ...
  rows.value = data.map(r => ({
    ...r,
    // keep original natural keys for safe updates
  ...
  rows.value.unshift(
    mode.value === 'life'
      ? {
          ...
          renewal_rate: null,
          // NEW fields
          agent_excess_rate: null,
          agent_renewal_rate: null,
          orig_carrier_name: null,
          orig_product_name: null
        }
      : {
          carrier_name: '',
          product_name: '',
...
async function save(row) {
  ...
  if (mode.value === 'life') {
    const payload = {
      ...
      agent_excess_rate: row.agent_excess_rate ?? null,
      agent_renewal_rate: row.agent_renewal_rate ?? null,

      // original keys for update
